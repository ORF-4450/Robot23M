...
/**
 * Driving command that feeds target speed and rotation (% power) to the
 * DriveBase. Used for single stick arcade style driving.
 */
public class ArcadeDrive extends CommandBase {
	 ...
	 * @param turnInPlace
	 *            True to enable turn in place (only rotation valye applies).
	 */
	public ArcadeDrive(DriveBase subsystem, DoubleSupplier power, DoubleSupplier rotation,
	 ...
	 * gears is finished. That reschedule means initialize() is called again. When
	 * the robot is disabled this command is interrupted but retained by the
	 * scheduler. When reenabled scheduler will call initialize to restart running
	 * this command. Commands only cease to exist (deleted by scheduler) when thier
	 * isFinished method returns true. All commands work like this.
	 */
	@Override
	 ...
	 * typically the joystick X & Y deflection values but can be any double
	 * provider.
	 */
	@Override
	public void execute() {
		...
		LCD.printLine(LCD_2, "power=%.3f  rot=%.3f tip=%b  (lpwr=%.3f) (rpwr=%.3f)   ", power, rotation, turnInPlace,
