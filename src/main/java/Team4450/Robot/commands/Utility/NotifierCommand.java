package Team4450.Robot.commands.Utility;
 ...
 * separate thread. Has no end condition if period != 0; either subclass it or
 * use {@link Command#withTimeout(double)} or
 * {@link Command#withInterrupt(BooleanSupplier)} to give it one. Modified by
 * Team 4450 so that Period == 0 runs runnable one time then command ends.
 *
 * <p>
...
public class NotifierCommand extends CommandBase {
	 ...
	 *
	 * @param toRun
	 *            The runnable for the notifier to run.
	 * @param period
	 *            The period at which the notifier should run, in seconds. Zero runs
	 *            the runnable one time then the command ends.
	 * @param requirements
	...
	/**
	 * Creates a new NotifierCommand. Caller must schedule the command for it to
	 * execute.
	 *
	 * @param toRun
	 *            The runnable for the notifier to run.
	 ...
	 *
	 * @param run
	 *            True to run when disabled, false if not.
	 */
