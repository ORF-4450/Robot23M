...
public class AutoCurve2 extends CommandBase {
	...
	/**
	 * Automatically drive in a curve. This method will do tighter curves as it uses
	 * arcadeDrive method for the steering. You have to balance the power and curve
	 * parameters to get the turn you need.
	 *
	 * @param driveBase
	 ...
	 * @param target
	 *            Target angle to turn. If using angle, this is 0..180, - left, +
	 *            right. If using heading this is the target heading 0..359. If
	 *            using totalAngle, this is total degrees of rotation, - left, +
	 *            right.
	 * @param stop
	 ...
	 *            current direction, totalAngle: target is number of degrees of
	 *            rotation to turn.
	 *
	 ...
	 */
	public AutoCurve2(DriveBase driveBase, double power, double curve, double target, StopMotors stop, Brakes brakes,
		...
		Util.checkRange(curve, 1.0, "curve");
	...
	public void execute() {
		...
		if (pid == Pid.on) {
			...
			if (heading == Heading.heading)
				...
				yaw = RobotContainer.navx.getTotalYaw();
			else
				yaw = RobotContainer.navx.getYaw();

			// Our target is zero yaw error so we determine the difference between
		...
		{
			driveBase.arcadeDrive(power, curve, false);
