...
public class AutoRotate extends CommandBase {
	 ...
	 *
	 * Auto rotate the specified target angle from where the robot is currently
	 * pointing or rotate to a target heading.
	 *
	 * @param driveBase
	 *            The DriveBase subsystem used by this command to drive the robot.
	 * @param power
	 *            Max power for rotation. Power is always +.
	 * @param target
	 *            Target angle (-left, +right) to rotate from robot current
	 *            direction -180..+180, or target heading (0..359) to rotate to from
	 *            robot current heading. Target heading is always + and cannot be
	 *            more than 180 degrees away from current heading.
	 * @param pid
	 *            Off for simple rotation, On use PID controller to manage the
	 *            rotation slowing rotation as target is reached.
	 * @param heading
	 *            Off target is an angle, On target is a heading.
	 *
	 ...
	 *            different values to turn smoothly and accurately.
	 */
	public AutoRotate(DriveBase driveBase, double power, double target, Pid pid, Heading heading) {
		this.driveBase = driveBase;

		Util.consoleLog("pwr=%.2f  target=%.2f  pid=%s  hdg=%s", power, target, pid, heading);

		if (power <= 0)
			throw new IllegalArgumentException("power must be + non-zero");
	...
	public void execute() {
		...
		if (pid == Pid.on) {
			...
			Util.consoleLog("curve=%.2f  hdg=%.2f  yaw=%.2f  err=%.2f  time=%.4f", curve, saveHeading, yaw,
					pidController.getError(), elapsedTime);
