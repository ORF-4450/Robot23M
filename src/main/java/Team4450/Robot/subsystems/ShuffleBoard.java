...
/**
 * This class hosts functions relating to communicating with the ShuffleBoard
 * driver station application. Primarily, it's periodic function handles the
 * regular update of the "LCL" panel's display of robot status information when
 * the robot is active.
 */
public class ShuffleBoard extends SubsystemBase {
	...
	// This method will be called once per scheduler run on the scheduler (main)
	// thread. Only
	...
	public void updateDS() {
		if (tracing)
			FunctionTracer.INSTANCE.enterFunction("ShuffleBoard.updateDS");
		...
		LCD.printLine(LCD_4, "utilLY=%.3f  utilLX=%.3f - utilRY=%.3f  utilRX=%.3f",
				RobotContainer.utilityPad.getLeftYDS().getAsDouble(), RobotContainer.utilityPad.getLeftX(),
				RobotContainer.utilityPad.getRightYDS().getAsDouble(), RobotContainer.utilityPad.getRightX());

		LCD.printLine(LCD_7, "Lrpm=%d - Rrpm=%d  Lmax vel=%.3f - Rmax vel=%.3f",
				RobotContainer.driveBase.leftEncoder.getRPM(), RobotContainer.driveBase.rightEncoder.getRPM(),
		...
		// RobotContainer.navx.getYaw(), RobotContainer.navx.getTotalYaw(),
		// RobotContainer.navx.getTotalYaw180(),
		// RobotContainer.navx.getTotalAngle(), RobotContainer.navx.getHeading());

		// LCD.printLine(LCD_10, "ltt=%d labspos=%d ldeg=%.1f rtt=%d rabspos=%d
		// rdeg=%.1f",
		...
		// LCD.printLine(LCD_10, "ccpos=%d ccabspos=%d ccdeg=%.1f ccmax=%.1f lpos=%d
		// labspos=%d ldeg=%.1f lmax=%.1f",
		...
		if (tracing)
			FunctionTracer.INSTANCE.exitFunction("ShuffleBoard.updateDS");
	}
	...
	/**
	 * Switch tab on shuffleboard display by tab name. Will create the tab if it
