...
public class DriveBase extends SubsystemBase {
	...
	public DriveBase() {
		...
		// work and therefore the value field for the Talon in LW display is always
		// zero,
		...
		// Configure CAN Talons with appropriate inversion determined by testing on
		// robot.
		// These inversions don't work under simulation due to how inversion works on a
		// real
		...
		// The real robot has to invert the encoders as needed so both encoder read
		// increasing
		// values going forward. This should be the same for simulation, but it did not
		// work right so invert as needed under sim. I am sure this is due to a mistake
		// in how
		...
		// that time. When it trips, the watchdog will print to the console some
		// somewhat
		...
		// We do this because we use curvatureDrive in our auto routines for both
		// straight and
		// curved/rotation drivng. We use quickturn feature of curvatureDrive for
		// rotation. If
		// don't set this threshold to zero, curvatureDrive will compute a compensation
		// factor
		// during quickturn which it then applies to next call to curvatureDrive without
		// quickturn.
		// This causes that first call to curvatureDrive without quickturn to compute
		// power values
		// that are incompatible with what we are typically doing, which is to drive
		// straight after
		// a rotation.

		// robotDrive.setQuickStopThreshold(0); Removed from 2022 WPILIb. Substitute not
		// yet identified.
		...
		// odometer = new
		// DifferentialDriveOdometry(RobotContainer.navx.getTotalYaw2d());
	...
	// Simulation classes help us simulate our robot. Our SRXMagneticEncoderRelative
	// class
	// is not compatible with the Wpilib EncoderSim so create regular Encoder
	// objects which
	// which are compatible with EncoderSim. We then pass the dummy encoders into
	// the SRX
	// encoders which will use the dummy encoders during sim. During sim, the
	// EncoderSim
	// classes drive the dummy encoders which then drive our SRX encoders.
	private void configureSimulation() {
		Util.consoleLog();

		// double distancePerTickMeters = Math.PI *
		// Units.inchesToMeters(DRIVE_WHEEL_DIAMETER) /
		// SRXMagneticEncoderRelative.TICKS_PER_REVOLUTION;
		...
		driveSim = new DifferentialDrivetrainSim(DCMotor.getCIM(2), // 2 CIM motors on each side of the drivetrain.
		...
		// the Field2d class lets us visualize our robot in the simulation GUI. We have
		// to
	...
	public void periodic() {
		if (tracing)
			FunctionTracer.INSTANCE.enterFunction("DriveBase.periodic");

		// Update the odometer tracking robot position on the field. We have to track
		// the
		// cumulative encoder counts since at any time we can reset the encoders to
		// facilitate
		// driving functions like auto drive, alt driving mode and more. Odometer wants
		// counts
		...
		if (robot.isEnabled() && RobotBase.isSimulation()) {
			...
			Util.consoleLog("cld=%.3f  crd=%.3f  px=%.3f py=%.3f prot=%.1f tangle=%.1f simhdg=%.1f yaw=%.1f",
					cumulativeLeftDist, cumulativeRightDist, pose.getX(), pose.getY(), pose.getRotation().getDegrees(),
					-RobotContainer.navx.getTotalAngle2d().getDegrees(), -driveSim.getHeading().getDegrees(),
					RobotContainer.navx.getYaw());
		}

		// Update the sim field display with the current pose, or position and direction
		// of the robot, after we
		// updated that pose above.
		if (RobotBase.isSimulation())
			fieldSim.setRobotPose(pose);

		if (tracing)
			FunctionTracer.INSTANCE.exitFunction("DriveBase.periodic");
	}

	// Updates simulation data *after* to each periodic() (above) call when under
	// simulation. Then all other
	// commands have thier execute funtion called.
	@Override
	public void simulationPeriodic() {
		if (robot.isEnabled()) {
			if (tracing)
				FunctionTracer.INSTANCE.enterFunction("DriveBase.simulationPeriodic");
			...
			// TalonSRX implemment inversion by switching the output leads of the
			...
			// Util.consoleLog("linvert=%b rinvert=%b", LRCanTalon.getInverted(),
			// RRCanTalon.getInverted());
			...
			// RobotContainer.canCoder.setSimValues(driveSim.getLeftPositionMeters(),
			...
			// We change the sign because the sign convention of Rotation2d is opposite of
			// our convention used
			...
			if (tracing)
				FunctionTracer.INSTANCE.exitFunction("DriveBase.simulationPeriodic");
		}
	}
	 ...
	 *
	 * @param leftPower
	 *            Left % power setting -1.0 to +1.0.
	 * @param rightPower
	 *            Right % power setting -1.0 to +1.0.
	 * @param squareInputs
	 *            True reduces sensitivity at low speeds.
	 */
	public void tankDrive(double leftPower, double rightPower, boolean squareInputs) {
		robotDrive.tankDrive(leftPower, rightPower, squareInputs);

		// Util.consoleLog("l=%.3f m=%.3f r=%.3f m=%.3f", leftPower, LRCanTalon.get(),
		// rightPower, RRCanTalon.get());
	}

	/**
	 * Tank drive function. Passes left/right power values to the robot drive.
	 * Should be called every scheduler run by the Drive command. Uses
	 * SlewRateLimiter filter to delay inputs to smooth out control. This appears as
	 * a delay applying power but desired power is reached in the time specific in
	 * SlewRateLimiter constructor. Robot may appear slow to respond then accelerate
	 * abrutly as full input is reached In theory, the CAN Talon ramp rate we set
	 * globally hasw the same effect, but it did not seem to make much difference.
	 * Here we are layering ramp rate and slewratelimiting to tone down the
	 * responsiveness of the driving controls.
	 *
	 * @param leftSpeed
	 *            Left % power setting -1.0 to +1.0.
	 * @param rightSpeed
	...
	public void tankDriveLimited(double leftPower, double rightPower) {
		...
		// Util.consoleLog("l=%.2f m=%.2f r=%.2f m=%.2f", leftPower, LRCanTalon.get(),
		// rightPower, RRCanTalon.get());
	}
	 ...
	 *
	 * @param power
	 *            % Power setting -1.0 to +1.0.
	 * @param rotation
	 *            Rotation rate -1.0 to +1.0. Clockwise us +.
	 * @param quickTurn
	 *            True causes quick turn (turn in place).
	 */
	public void curvatureDrive(double power, double rotation, boolean quickTurn) {
		...
		// Util.consoleLog("pwr=%.3f rot=%.3f lget=%.4f(%.4fv) rget=%.4f(%.4fv)", power,
		// rotation,
	 ...
	 *
	 * @param power
	 *            % Power setting -1.0 to +1.0, positive is forward.
	 * @param rotation
	 *            Rotation rate -1.0 to +1.0, positive is clockwise.
	 * @param squareInputs
	 *            When set reduces sensitivity a low speeds.
	 */
	...
	/**
	 * It is not clear which of these two methods is correct. The above method
	 * should work because we globally reverse motors to make robot drive forward
	 * with + power or voltages. However, negation of right side voltage appears to
	 * be necessary for trajectory following with voltages. So this method is
	 * provided to make the trajectory following Command work. More investigation
	 * needed... Update: With 2022 wpilib, this routine is incorrect. Suspect all
	 * this is related to the built-in inversion of right side in the wpilib
	 * robotdrive class and the confusion it caused us in setting up the motor
	 * inversions. That built-in inversion is gone for 2022. All this has to be
	 * retested for 2022 code migration.
	 */
	...
	// or a talon has low voltage value, it did not get recognized by the RR on
	// start up.
	 ...
	 *
	 * @param seconds
	 *            Number of seconds from zero to full output. zero disables.
	 */
	...
	public String GetCANTalonStatus() {
		return String.format("%.1f/%.1f  %.1f/%.1f  %.1f/%.1f  %.1f/%.1f  %.1f/%.1f  %.1f/%.1f",
				LFCanTalon.getMotorOutputVoltage(), LFCanTalon.getStatorCurrent(), LRCanTalon.getMotorOutputVoltage(),
				LRCanTalon.getStatorCurrent(), RFCanTalon.getMotorOutputVoltage(), RFCanTalon.getStatorCurrent(),
	 ...
	 *
	 * @param enabled
	 *            True to turn on voltage compensation, false to turn off.
	 */
	...
	/**
	 * Reset drive wheel encoders as in resetEncoders(), except that the encoder
	 * simulation is reset to zero. This is used to start a test run over allowing
	 * an auto program to run, be disabled, then restarted to run again from the
	 * starting point. The sim needs a special reset to support this. Should only be
	 * called from auto program initialization.
	 */
	...
	/**
	 * Reset the drive wheel encoders to zero with time delay. There can be a
	 * significant delay between calling for encoder reset and the encoder returning
	 * zero. Sometimes this does not matter and other times it can really mess
	 * things up if you reset encoder but at the time you next read the encoder for
	 * a measurement (like in autonomous programs) the encoder has not yet been
	 * reset and returns the previous count. This method resets and delays 36ms
	 * which sould cause the next read of the reset encoders to return zero. The
	 * response delay is ~20ms plus ~10ms each send delay. So we now wait 36ms total
	 * to let resets complete.
	 */
	public void resetEncodersWithDelay() {
		...
		// Set encoders to update every 20ms just to make sure. These calls take 10ms
		// each so we
		// just do them once.
	...
	/**
	 * Get current pose from odometer. Pose distances in meters. Pose X is distance
	 * along the long side of the field from your driver station wall. Y is distance
	 * along the short side of the field starting on the left. Angle is cumulative
	 * referenced from zero as pointing directly at the opposition driver station
	 * wall, + is left, - is right of that zero alignment.
	 *
	 * @return Current pose.
	 */
	...
	/**
	 * Reset odometer to new position and cumulative angle. Pose x,y distances in
	 * meters, as described in getOdometerPose() doc.
	 *
	 * @param pose
	 *            New starting pose.
	 * @param heading
	 *            Heading of robot (cumulative angle) in degrees.
	 */
	public Pose2d resetOdometer(Pose2d pose, double heading) {
		...
		// odometer = new DifferentialDriveOdometry(pose.getRotation(), new
		// Pose2d(pose.getX(), pose.getY(), pose.getRotation()));

		Pose2d newPose = odometer.getPoseMeters();

		Util.consoleLog("x=%.3f  y=%.3f  angle=%.1f", newPose.getX(), newPose.getY(),
	...
	public double getAvgEncoderDist() {
		return (leftEncoder.getDistance(DistanceUnit.Meters) + rightEncoder.getDistance(DistanceUnit.Meters)) / 2;
	}
	/**
	 * Left encoder counts.
	 *
